...
async function run() {
  ...
  try {
    ...
    await frame.evaluate(() => window.safeLoginTest.setChain(11155111))
    await frame.click("#login")
    await frame.locator("button::-p-text(Use Safe approvals)").click()
    await frame.waitForFunction(
      () => window.safeLoginTest.state.safeRequests === 1,
    )
    await frame.locator("button::-p-text(Cancel)").click()
    await frame.waitForFunction(
      () => !document.querySelector('[role="dialog"]'),
    )
    await frame.evaluate(() => window.safeLoginTest.approveSafe())
    assert.equal(
      await frame.evaluate(() => window.safeLoginTest.state.signed.length),
      0,
    )
    await frame.click("#login")
    await frame.locator("button::-p-text(Use Safe approvals)").click()
    await frame.waitForFunction(
      () => window.safeLoginTest.state.safeRequests === 2,
    )
    await frame.evaluate(() => window.safeLoginTest.approveSafe())
    await frame.waitForFunction(
      () => window.safeLoginTest.state.signed.length === 1,
    )
    assert.equal(
      await frame.evaluate(
        () => window.safeLoginTest.state.signed[0].signature,
      ),
      "0x",
    )
    assert.equal(
    ...
    console.log(
      "Safe login browser checks passed: modal stacking, wallet search, focus restoration, pairing cancellation, Safe approval retry and primary account isolation.",
    )
  } finally {
