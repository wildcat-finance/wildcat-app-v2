import React from "react"

import { Box, Typography } from "@mui/material"
import Link from "next/link"
import { useTranslation } from "react-i18next"

import { useBlockExplorer } from "@/hooks/useBlockExplorer"
import { COLORS } from "@/theme/colors"

export type WrapperFormProps = {
  isWrapping?: boolean
  message?: string
  txHash?: string
}

export const ErrorWrapperAlert = ({
  isWrapping,
  message,
  txHash,
}: WrapperFormProps) => {
  const { t } = useTranslation()
  const { getTxUrl } = useBlockExplorer()

  return (
    <Box
      sx={{
        width: { xs: "calc(100% - 32px)", md: "100%" },
        marginX: { xs: "auto", md: "0" },
        display: "flex",
        flexDirection: "column",
        gap: "4px",
        padding: "16px",
        borderRadius: "13px",
        backgroundColor: COLORS.remy,
        position: "relative",
        marginTop: "24px",
      }}
    >
      {/* Triangle pointer */}
      <Box
        sx={{
          position: "absolute",
          top: "-12px",
          right: { xs: "145px", md: "55px" },
          width: 0,
          height: 0,
          borderLeft: "8px solid transparent",
          borderRight: "8px solid transparent",
          borderBottom: `12px solid ${COLORS.remy}`,
        }}
      />

      <Typography variant="text2" color={COLORS.dullRed}>
        {isWrapping
          ? t("marketDetails.lender.wrapDebtToken.error.titleWrap")
          : t("marketDetails.lender.wrapDebtToken.error.titleUnwrap")}
      </Typography>

      <Box sx={{ display: "flex", gap: "2px" }}>
        <Typography variant="text4" color={COLORS.dullRed08}>
          {message ||
            t("marketDetails.lender.wrapDebtToken.error.defaultMessage")}
        </Typography>

        {txHash && (
          <Link
            href={getTxUrl(txHash)}
            target="_blank"
            style={{
              display: "flex",
              color: COLORS.dullRed08,
              textDecorationLine: "underline",
            }}
          >
            <Typography variant="text4" color={COLORS.dullRed08}>
              {" "}
              {t("common.links.viewOnEtherscan")}
            </Typography>
          </Link>
        )}
      </Box>
    </Box>
  )
}
