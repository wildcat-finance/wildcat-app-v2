...
import { claimPartBuild } from "./partBuild"
import { ProviderThrottleError } from "./providerThrottle"
import { exportObjectExists, getExportObject, putExportObject } from "./storage"
...
async function buildPart(
  ...
  try {
    ...
    const cacheExists = await exportObjectExists(key)
    if (cacheExists) {
      await updateActiveJob(jobId, {
      ...
      // An earlier upload may have failed after recording metadata. The claim
      // gives this builder ownership of an absent object; stale metadata is
      // not evidence that the newly generated bytes are corrupt.
      await prisma.exportArtifact.deleteMany({ where: { key } })
      const rpc = new ExportRpcClient(request.chainId)
      ...
      await updateActiveJob(jobId, {
        phase: `saving_market_data_${index + 1}_of_${total}`,
      })
      try {
      ...
      // Storage's create-only upload chooses one immutable winner, including
      // overlapping deliveries of the same Workflow step. Record its bytes
      // only after publication, so failed uploads cannot poison later retries.
      await recordArtifact(key, ExportArtifactKind.Part, part)
    }
  } catch (error) {
    throw deterministicError(error)
  ...
  await prisma.exportPartBuild.deleteMany({ where: { key, jobId } })
  return key
}
...
async function assemble(
  ...
  // Retain the destination for cleanup even if cancellation interrupts the
  // gap between upload and artifact recording.
  await updateActiveJob(jobId, { artifactKey: key })
  let publishedBundle = bundle
  try {
    await putExportObject(key, bundle, "application/zip")
  } catch (error) {
    if (!(await exportObjectExists(key))) throw error
    publishedBundle = await getExportObject(key)
  }
  await recordArtifact(key, ExportArtifactKind.Bundle, publishedBundle, jobId)
