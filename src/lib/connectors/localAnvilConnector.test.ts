...
describe("localAnvilConnector", () => {
  ...
  // The e2e harness connects as a real Sepolia borrower it has no key for: anvil unlocks the
  // address (anvil_impersonateAccount) and the connector is told which one through localStorage.
  it("connects as an impersonated address, ahead of the account index", async () => {
    const impersonated = "0xE9aEE885aF757FF7f632000769046876814926E6"
    window.localStorage.setItem("anvil-account-index", "3")
    window.localStorage.setItem("anvil-impersonate-address", impersonated)
    const c = make()
    expect((await c.connect()).accounts[0]).toBe(impersonated)
    expect((await c.getAccounts())[0]).toBe(impersonated)
    const provider = await c.getProvider()
    await expect(provider.request({ method: "eth_accounts" })).resolves.toEqual(
      [impersonated],
    )
    // …and it stays authoritative even after an explicit index connect.
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    await c.connect({ withAccountIndex: 2 } as any)
    expect((await c.getAccounts())[0]).toBe(impersonated)
  })

  it("ignores a malformed impersonation address and falls back to the index", async () => {
    window.localStorage.setItem("anvil-account-index", "3")
    window.localStorage.setItem("anvil-impersonate-address", "not-an-address")
    const c = make()
    expect((await c.getAccounts())[0]).toBe(ANVIL_DEFAULT_ACCOUNTS[3])
  })
