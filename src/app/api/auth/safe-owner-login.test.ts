...
import { ISafe__factory as ISafeFactory } from "@wildcatfi/wildcat-sdk/dist/typechain"
...
const provider = {
  ...
  call: jest
    .fn()
    .mockResolvedValue(utils.defaultAbiCoder.encode(["bool"], [false])),
} as unknown as ReturnType<typeof getProviderForServer>
const contract = {
  getOwners,
  "isValidSignature(bytes32,bytes)": jest.fn().mockResolvedValue("0xffffffff"),
  "isValidSignature(bytes,bytes)": jest.fn().mockResolvedValue("0xffffffff"),
} as unknown as ReturnType<typeof ISafeFactory.connect>
...
beforeAll(() => {
  ...
  jest.spyOn(ISafeFactory, "connect").mockReturnValue(contract)
})
  ...
  return new NextRequest("http://localhost/api/auth/login", {
    ...
    body: JSON.stringify({
      ...
      signature: await wallet.signMessage(
        getLoginSignatureMessage(account, timeSigned, messageChainId),
      ),
    }),
  })
}
...
it("retains API login for a signature validated by the full Safe verification path", async () => {
  jest
    ...
    .mockResolvedValueOnce(utils.defaultAbiCoder.encode(["bool"], [true]))
