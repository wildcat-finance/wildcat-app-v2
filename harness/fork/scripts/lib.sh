...
# Pin file: harness/fork/pins.json, overridable with $WILDCAT_PINS.
PINS="${WILDCAT_PINS:-$HERE/pins.json}"
[ -f "$PINS" ] || { echo "missing pin file $PINS" >&2; exit 1; }
...
pin_or(){ jq -r "$1 // \"$2\"" "$PINS"; }                      # optional key with a default
...
# Each variant serves its own fork subgraph on the shared fork node: v2.5 keeps wildcat-sepolia-fork,
# main uses wildcat-sepolia-fork-v218 (different subgraph version, incompatible schema).
FORK_NAME="$(pin_or .subgraph.forkName wildcat-sepolia-fork)"; FORK_CHAIN=fork
APP_PORT="$(pin_or .app.port 3000)"
# NB: jq's `//` treats `false` as absent, so `.harness.ownsStack // true` would read a pinned
# `false` as `true` and silently disarm the guard below. Test for the literal instead.
OWNS_STACK="$(jq -r 'if .harness.ownsStack == false then "false" else "true" end' "$PINS")"

# The docker compose project (wildcat-fork) is shared by every worktree, and fork-destroy.sh removes
# EVERY subgraph on the fork chain. Scripts that mutate shared containers, volumes or the app DB call
# this so a non-owning variant cannot tear the other variant's stack down by accident.
require_stack_owner(){
  [ "$OWNS_STACK" = true ] && return 0
  [ "${HARNESS_ALLOW_MUTATE:-}" = 1 ] && { log "HARNESS_ALLOW_MUTATE=1: proceeding on shared stack"; return 0; }
  fail "$(basename "${0:-script}") mutates the shared 'wildcat-fork' stack, which this app does not own ($PINS: .harness.ownsStack=false).
  Use scripts/attach.sh to write .env.fork against the running stack.
  If the owning variant is idle and you really mean it, re-run with HARNESS_ALLOW_MUTATE=1."
}
