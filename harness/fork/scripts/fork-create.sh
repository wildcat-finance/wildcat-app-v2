...
net=$(gql "$FORK_GQL" '{ indexerDeployments(first:1){ network chainId } }' | jq -r '.data.indexerDeployments[0] | "\(.network) \(.chainId)"')
[ "$net" = "sepolia 11155111" ] || fail "fork IndexerDeployment reports '$net', expected 'sepolia 11155111'"
