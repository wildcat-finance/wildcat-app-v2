 ...
 *
 * GENERATION: LEN-28…31 run on the PINNED pre-fork pair (pins.markets.wrapperMarket / .wrapper),
 * which is a LEGACY (pre-V2.5) market served by the v1 wrapper factory. The V2.5-generation
 * lifecycle, post-hoc deployment and closure-with-wrapped-supply live in
 * ./wrappers-deployment.spec.ts (UAT sheet 6, WRP-01…05). Generation semantics, the facade
 * routing rules and the scaled-state arithmetic these tests assert are documented and cited in
 * ./wrapperLib.ts.
 */
test.describe.serial("lender flows: wrapper & transfers (LEN-28…33)", () => {
  ...
  // The app resolves the market's ACTIVE wrapper through the SDK's configured factory, which is
  // the V2.5 FACADE — a newer instance can exist than the subgraph-known pin, and a legacy market
  // resolves only by the facade's v1 forward. Resolved from the factory in setup and re-checked
  // against the page's own wrapper link in LEN-28.
  ...
  /**
   * The scaled amount the market's transfer moves for `assets`. V2.5 markets declare floor
   * scaling (`scaleAmountDown`); pre-V2.5 markets round half-up and the v1 wrapper's execution
   * arithmetic matches that. Returns both candidates so a mismatch names the observed rounding
   * instead of failing with two opaque numbers.
   */
  const scaledCandidates = (assets: bigint, scaleFactor: bigint) => ({
    floor: scaleAmountDown(assets, scaleFactor),
    halfUp: scaleAmountHalfUp(assets, scaleFactor),
  })

  const roundingOf = (
    observed: bigint,
    candidates: { floor: bigint; halfUp: bigint },
  ) => {
    if (observed === candidates.floor && observed === candidates.halfUp)
      return "floor==half-up"
    if (observed === candidates.floor) return "floor"
    if (observed === candidates.halfUp) return "half-up"
    return "neither"
  }

  /** The scaled amount THIS generation's market transfer must move for `assets`. */
  const expectedScaled = (candidates: { floor: bigint; halfUp: bigint }) =>
    generation.roundsFloor ? candidates.floor : candidates.halfUp

  /**
   * Wait for a wrapper transaction's DURABLE on-chain effect (the success modal unmounts on
   * refetch), and decorate a timeout with whatever the app rendered instead.
   *
   * HARNESS BUG THAT FIXMED LEN-28…31: the pin-era body probed for an error message with
   * `locator.innerText().catch(() => null)` INSIDE the poll. `innerText()` AUTO-WAITS, so with no
   * error on screen every poll iteration blocked for the default 30 s timeout and the 120 s poll
   * expired having read the chain only a handful of times — while the wrap had in fact succeeded
   * (the `deposit(uint256,address)` transaction sits in the block right after the approval).
   * Never auto-wait for the ABSENCE of an element inside a poll; probe for it only after the poll
   * has failed, with an explicit short timeout.
   */
  const awaitOnChain = async (
    ...
    try {
      ...
      const appError = await page
        .getByText(
          /oops|something goes wrong|something went wrong|try again|reverted|transaction failed/i,
        )
        .first()
  ...
  test("setup: hygiene and a lender position on the wrapper market", async () => {
    ...
    // Generation + discovery, established BEFORE any assertion depends on a generation-specific
    // API. The pinned pair is pre-V2.5, so `registeredWrapper()` does not exist on the market and
    // only the facade's v1 forward answers (wrapperLib.ts header; COMPATIBILITY.md "Wrapper
    // Compatibility").
    generation = await marketGeneration(market)
    const resolved = await wrapperForMarket(market)
    expect(
      resolved,
      "the configured factory resolves this market's wrapper",
    ).not.toBeNull()
    wrapper = resolved!
    // Self-clean: a crashed run can leave a wrapped position behind, and LEN-31 asserts a
    // zero-share end state. Redeem it chain-side before the UI lifecycle starts.
    const leftover = await wrapperRedeemAll(account0, wrapper)
    expect(await wrapperShares(wrapper, account0), "no wrapper shares held").toBe(
      0n,
    )
    const indexed = await indexedWrapperForMarket(market)
    attachAgreement("wrapper fixture (LEN-28…31)", {
      market,
      wrapper,
      pinnedWrapper: pinnedMarkets.wrapper.toLowerCase(),
      generation: generation.label,
      declaresRounding: generation.declaresRounding,
      registeredWrapper: generation.registeredWrapper,
      indexedFactory: indexed?.factory ?? null,
      wrapperSymbol: await wrapperSymbol(wrapper),
      wrapperTotalSupply: await wrapperTotalSupply(wrapper),
      leftoverSharesRedeemed: leftover,
    })
  })

  /*
   * LEN-28…31 were `test.fixme` on the pin-era harness ("the Wrap Tokens click is a silent
   * no-op"). Re-implemented here against the current build; the historical bodies' selectors were
   * re-verified against source (WrapperSection/index.tsx, WithdrawModal/*) and kept where still
   * correct. The pin-era no-op did NOT reproduce — see the authoring report. The remaining
   * pin-era observations are handled rather than worked around:
   *  - the app can use a NEWER wrapper instance than the pin: setup resolves the wrapper from the
   *    configured factory and LEN-28 cross-checks the page's own link against it;
   *  - the section can be re-selected mid-click by the page's own effect: `openSection` retries.
   */

  test("LEN-28: wrap market tokens into the 4626 wrapper", async ({ page }) => {
    const bal0 = await chain.marketBalance(market, account0)
    const scaled0 = await scaledBalanceOf(market, account0)
    const wrapperScaled0 = await scaledBalanceOf(market, wrapper)
    const shares0 = await wrapperShares(wrapper, account0)

    await step(page, "generation-specific discovery", async () => {
      // Legacy markets carry NEITHER V2.5 wrapper API. Asserting the revert is the point: it is
      // why discovery must not call `registeredWrapper()` on this generation
      // (COMPATIBILITY.md "Wrapper Compatibility"; Wildcat4626WrapperFactory.sol:119-132).
      expect(generation.label, "pinned wrapper market generation").toBe("legacy")
      expect(
        generation.declaresRounding,
        "a pre-V2.5 market declares no scaled-transfer rounding",
      ).toBe(false)
      expect(
        generation.registeredWrapper,
        "registeredWrapper() is unavailable on a legacy market",
      ).toBeNull()
      const indexed = await indexedWrapperForMarket(market)
      expect(indexed, "the wrapper is indexed").not.toBeNull()
      expect(
        indexed!.factory.generation,
        "indexed under the v1 wrapper-factory generation",
      ).toBe("v1")
      expect(indexed!.id.toLowerCase(), "indexed wrapper == resolved wrapper").toBe(
        wrapper,
      )
    })
    ...
    await step(page, "banner mirrors on-chain balances", async () => {
      // The page's own wrapper link must agree with the factory-resolved instance.
      ...
      expect(
        wrapperMatch?.[1].toLowerCase(),
        "the page links the factory-resolved wrapper",
      ).toBe(wrapper)
    ...
    const wrapperScaled1 = await scaledBalanceOf(market, wrapper)
    // The market's `_transfer` accrues and WRITES state, so the stored scale factor read now (no
    // other market write has intervened) is the one the wrap scaled with.
    const scaleFactor = await marketScaleFactor(market)
    const sharesDelta = shares1 - shares0
    const valueOfNewShares = await wrapperConvertToAssets(wrapper, sharesDelta)
    const tolerance = parseUnits("0.01", decimals)
    const candidates = scaledCandidates(wrapAmount, scaleFactor)

    expect(sharesDelta > 0n, "v-token received").toBe(true)
    // SCALED deltas are interest-invariant: only a transfer can move them. This is the check that
    // separates a real wrap from the market token's per-block rebase.
    expect(
      scaled0 - scaled1,
      "the lender's scaled position fell by exactly the shares minted",
    ).toBe(sharesDelta)
    expect(
      wrapperScaled1 - wrapperScaled0,
      "the wrapper's scaled backing rose by exactly the shares minted",
    ).toBe(sharesDelta)
    // Generation-aware rounding: the shares minted are the market transfer's scaling of the
    // wrapped amount. V2.5 markets floor (`scaleAmountDown`); pre-V2.5 markets round HALF-UP —
    // verified on this fork by decoding the wrapper deposits/withdrawals and comparing the scaled
    // delta against both candidates at the mined block. Exact, not a tolerance.
    const rounding = roundingOf(sharesDelta, candidates)
    expect(
      sharesDelta,
      `shares minted = the ${
        generation.roundsFloor ? "floor" : "half-up"
      } scaling of ${wrapAmount} at scaleFactor ${scaleFactor} (floor=${
        candidates.floor
      }, half-up=${candidates.halfUp}, observed rounding=${rounding})`,
    ).toBe(expectedScaled(candidates))
    expect(
    ...
    attachAgreement("LEN-28 wrap", {
      ...
      wrapped: wrapAmount,
      sharesDelta,
      lenderScaledDrop: scaled0 - scaled1,
      wrapperScaledGain: wrapperScaled1 - wrapperScaled0,
      scaleFactor,
      scaledFloor: candidates.floor,
      scaledHalfUp: candidates.halfUp,
      observedRounding: rounding,
      valueOfNewShares,
  ...
  test("LEN-29: shares are non-rebasing; unwrap returns amount incl. interest", async ({
    page,
  }) => {
    ...
    await step(page, "unwrap an exact market-token amount", async () => {
      ...
      await awaitOnChain(
        page,
        "unwrap returns market tokens",
        async () =>
          (await chain.marketBalance(market, account0)) >
          bal0 + unwrapAmount / 2n,
      )
    })
    ...
    const valueOfSpentShares = await wrapperConvertToAssets(wrapper, sharesSpent)
    const tolerance = parseUnits("0.05", decimals)
    const candidates = scaledCandidates(unwrapAmount, scaleFactor)
    const rounding = roundingOf(sharesSpent, candidates)

    expect(
      absDiff(bal1 - bal0, unwrapAmount) <= tolerance,
      `received the requested market tokens: ${bal1 - bal0}`,
    ).toBe(true)
    // Scaled arithmetic: the shares burned are exactly the scaled tokens the wrapper's transfer
    // credited to the lender — interest dust cannot inflate a scaled balance.
    expect(
      scaled1 - scaled0,
      "the lender's scaled position rose by exactly the shares burned",
    ).toBe(sharesSpent)
    expect(
      sharesSpent,
      `shares burned = the ${
        generation.roundsFloor ? "floor" : "half-up"
      } scaling of ${unwrapAmount} at scaleFactor ${scaleFactor} (floor=${
        candidates.floor
      }, half-up=${candidates.halfUp}, observed rounding=${rounding})`,
    ).toBe(expectedScaled(candidates))
    ...
    attachAgreement("LEN-29 unwrap", {
      ...
      received: bal1 - bal0,
      sharesSpent,
      lenderScaledGain: scaled1 - scaled0,
      sharesAtWrapRate,
      scaleFactor,
      observedRounding: rounding,
      rateAtWrap,
  ...
  test("LEN-30: withdraw flow auto-unwraps the wrapped remainder", async ({
    page,
  }) => {
    const direct = await chain.marketBalance(market, account0)
    const wrapped = await wrapperMaxWithdraw(wrapper, account0)
    const sharesBefore = await wrapperShares(wrapper, account0)
    expect(wrapped > parseUnits("25", decimals), "wrapped position left").toBe(
      true,
    )
    ...
    // (MarketActions/index.tsx:97-102 — `marketBalance + wrapper.maxWithdraw`). The wrapper
    // account state is a SEPARATE query from the market account, so a read taken the instant the
    // page settles can still show direct-only; poll, and re-read BOTH sides each iteration
    // because market tokens rebase while we look at them.
    await expect
      .poll(
        async () =>
          absDiff(
            (await readAvailableToWithdraw(page)).raw,
            (await chain.marketBalance(market, account0)) +
              (await wrapperMaxWithdraw(wrapper, account0)),
    ...
    // Complete the UNDERLYING request/claim path: unwrapping only yields market tokens
    // (ERC4626_WRAPPER.md "Mental Model"), so the exit is only proven once the queued batch is
    // claimed for the underlying asset — through the app, not chain-side.
    let claimed = 0n
    await step(page, "claim the queued batch for the underlying", async () => {
      ...
      expect(claimed >= amountTyped - 10n, "batch fully payable").toBe(true)
    ...
    attachAgreement("LEN-30 auto-unwrap withdraw", {
      ...
      combinedAvailableOnPage: available.raw,
      sharesBefore,
      sharesAfter,
  ...
  test("LEN-31: unwrap the full wrapped balance to zero", async ({ page }) => {
    const sharesBefore = await wrapperShares(wrapper, account0)
    ...
    await step(page, "Max fill + unwrap everything", async () => {
      ...
      // cannot leave share dust behind (WrapperSection/index.tsx:295-299).
      ...
      await awaitOnChain(
        page,
        "full unwrap burns every share",
        async () => (await wrapperShares(wrapper, account0)) === 0n,
      )
    })

    const bal1 = await chain.marketBalance(market, account0)
    const scaled1 = await scaledBalanceOf(market, account0)
    expect(await wrapperShares(wrapper, account0), "zero shares remain").toBe(0n)
    // No unintended residual position anywhere: the redeem is exact-in over every share, so the
    // whole scaled position came back and the wrapper keeps no claim for this holder.
    expect(
      scaled1 - scaled0,
      "the lender's scaled position rose by exactly the shares redeemed",
    ).toBe(sharesBefore)
    expect(
      await wrapperMaxRedeem(wrapper, account0),
    ...
    attachAgreement("LEN-31 unwrap all", {
      ...
      received: bal1 - bal0,
      lenderScaledGain: scaled1 - scaled0,
      wrapperTotalSupplyAfter: await wrapperTotalSupply(wrapper),
    })
  })
