 ...
 * (LEN-05..12, LEN-32). The classification below mirrors the app at
 ...
 *
 * The oracle has to follow the app or LEN-06's counts will never reconcile — see
 * `onboardingMode` below.
 */
...
/**
 * Mirror of `utils/marketType.ts#isFrontendVisibleMarket`: the app "tolerates periodic market
 * data without exposing periodic UX", so every list hook drops PeriodicTerm V2 markets before
 * rendering. Without this the oracle over-counts by exactly the number of open periodic markets
 * on the fork (6 at the time of writing) and LEN-05/LEN-06 can never reconcile.
 */
const isFrontendVisible = (m: SubgraphMarket) =>
  m.version !== "V2" || m.hooks?.kind !== "PeriodicTerm"

/** Blacklist mirrored from src/utils/constants.ts:19-28 (EXCLUDED_MARKETS / EXCLUDED_BORROWERS). */
 ...
 *
 * src/utils/constants.ts:14-17 empties both lists when `shouldShowExcludedMarkets` —
 * `NODE_ENV === "development"` or the secretsite host — so the answer depends on HOW the harness
 * serves the app: `npm run dev:fork` (next dev) lists them, the production `next start` the
 * boards run drops them. On main the drop happens in the SUBGRAPH QUERY, not just client-side:
 * EXCLUDED_MARKETS_FILTER (constants.ts:41-49) is spliced into the lender catalogue query at
 * src/app/[locale]/lender/hooks/useLendersMarkets.ts:77 (and the borrower ones at
 * getMaketsHooks/useGetOthersMarkets.ts:41, useGetBorrowerMarkets.ts:49), with a second
 * client-side pass over EXCLUDED_MARKETS in AllMarketsSection/index.tsx:160 — so the row is never
 * part of any bucket the UI counts. The oracle just did not model it.
 *
 * NODE_ENV is inlined at build time, so probe the build instead: `next dev` serves
 * /_next/static/development/_buildManifest.js, a production build 404s it (verified against the
 * board's server on :3001).
 */
...
export const fetchAllMarkets = async (): Promise<SubgraphMarket[]> => {
  const rows = (
    await gql<{ markets: SubgraphMarket[] }>(
      `{ markets(first: 1000) {
        id name isClosed version annualInterestBips
        withdrawalBatchDuration borrower asset { symbol address }
        hooks { id kind providers { isApproved pullProviderIndex } }
  ...
  ).markets.filter(isFrontendVisible)
  if (!(await appHidesExcludedMarkets())) return rows
  return rows.filter(
    (m) =>
 ...
 *
 * The app uses plain `hasActivePullRoleProvider` (utils/marketCapabilities.ts), which counts ANY
 * approved provider holding a pull slot, access lists included — so a market whose only pull
 * provider is a borrower-run allowlist counts as "self". The oracle has to follow the app under
 * test or LEN-06's counts never reconcile.
 */
 ...
 * (AllMarketsSection/index.tsx:156-163), which is a SUPERSET of this: the SDK ORs the indexed
 * entry with live lens state — `hadSubgraphEntry || role !== Null || marketBalance > 0 || ...`
 * (@wildcatfi/wildcat-sdk/dist/account/index.js:57-64). A wallet merely granted a credential on a
 * market it never funded is therefore "mine" to the app and "other" here. Nothing on the fork is
 * in that state today; if a bucket count is ever off by one in the self/manual direction, check
 * that before the blacklist above.
 */
 ...
 * The ToU prompt closes on Escape; the KYB "How Wildcat checks this profile" explainer is
 * rendered with no onClose, so Escape does nothing and it has to be acknowledged through its own
 * button — the hardened shape from borrowerflows/onboarding.spec.ts:282-299.
 */
