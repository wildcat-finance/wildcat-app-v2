...
/**
 * Live delinquency state — `currentState()`, i.e. the contract's own projection to the
 * current block (lib/abis.ts#marketStateV2Abi). This is the value the app ends up rendering:
 * useLendersMarkets.ts:169-242 refreshes every MarketAccount through the lens
 * (refreshMarketAccountsV2LiveDataSafe -> MarketAccount.updateWithLiveData), which assigns
 * market.timeDelinquent / market.isDelinquent from the same view
 * (@wildcatfi/wildcat-sdk/dist/market.js:782-783). The subgraph's own values are as-of-last-event
 * and can be far off in BOTH directions once the timer has run.
 */
export const marketDelinquency = async (market: Address) => {
  const state = await chain.publicClient
    .readContract({
      address: market,
    ...
    .catch(() =>
      // V1 markets carry no protocolFeeBips, so the V2 tuple fails to decode.
      chain.publicClient.readContract({
        address: market,
        abi: marketStateV1Abi,
        functionName: "currentState",
      }),
    )
  return {
...
/**
 * `depositUpTo` simulation whose ABI also carries the ACCESS-CONTROL revert selectors, so the
 * caller learns WHICH prerequisite failed instead of "it reverted".
 *
 * OpenTermHooks.onDeposit (v2.5 src/access/OpenTermHooks.sol:262-302) checks, in order:
 * blocked → minimum deposit → credential, and WildcatMarket._depositUpTo calls the hook BEFORE
 * `asset.safeTransferFrom` (src/market/WildcatMarket.sol:72-75) — so an access revert says
 * nothing about the caller's balance/allowance. Negative tests must assert those separately.
 */
const depositAccessAbi = [
  {
    type: "function",
    name: "depositUpTo",
  ...
  { type: "error", name: "NotApprovedLender", inputs: [] },
  { type: "error", name: "DepositBelowMinimum", inputs: [] },
  { type: "error", name: "NullMintAmount", inputs: [] },
  { type: "error", name: "MaxSupplyExceeded", inputs: [] },
  { type: "error", name: "DepositToClosedMarket", inputs: [] },
] as const

export const simulateDepositAccess = async (params: {
  account: Address
  market: Address
  amount: bigint
...
export const ensureNoMlaAcknowledged = async (
  ...
  const acknowledged = async () => (await (await fetch(ackUrl)).json()) !== null
 ...
 * setSection(TRANSACTIONS | STATUS) every time the wallet/access inputs settle
 * — src/app/[locale]/lender/market/[address]/page.tsx:349-382 — and the sidebar
 * collapses to skeletons whenever the indexed query flips isLoading
 * (page.tsx:349-351 -> LenderMarketSidebar/index.tsx:84-100). Under test-mode
 * polling (2 s indexed / 3 s live, src/config/polling.ts) that lands every
 * couple of seconds, so a single fire-and-forget click is routinely undone
 * before the section renders (LEN-15 board failure: the click landed mid
 * skeleton and the page came back on Deposit & Withdraw). Pass `until` — a
 * locator only the target section renders — and the click is retried until the
 * choice sticks.
 */
