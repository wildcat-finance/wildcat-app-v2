...
test.describe.serial("lender flows: deposit & withdraw (LEN-15…22)", () => {
  ...
  test("LEN-15: self-onboarding market — minimum enforced, approve + deposit, 1:1 mint", async ({
    ...
    // is one label on Status & Details: v2.5's upstream taxonomy (SDK 3.2.5+ / 121172ad) renders
    // "Lender Onboarding: Open", while main renders the access rows directly ("Deposit Access:
    // Open Deposits"). Everything this case is FOR — the minimum-deposit gate, the approve +
    // deposit round trip and the 1:1 mint — is identical on both, and main had no other coverage
    // of a lender deposit through the modal.
  ...
  test("LEN-17: balance rebases upward at the configured APR", async ({
    ...
    await gotoMarket(page, market)
    await ensureConnected(page, account0)
    const rebased = await chain.marketBalance(market, account0)
    const available = await waitAvailableToWithdraw(page, rebased)
    expect(available.raw, "page shows the rebased balance").toBe(rebased)
  ...
  test("LEN-21: full exit — entire balance withdrawn and claimed, lender leaves", async ({
    ...
    await gotoMarket(page, market)
    await ensureConnected(page, account0)
    const onChainBalance = await chain.marketBalance(market, account0)
    const available = await waitAvailableToWithdraw(page, onChainBalance)
    expect(available.raw).toBe(onChainBalance)
    ...
    await step(page, "queue the entire balance via the Max chip", async () => {
      ...
      await expect
        .poll(async () => chain.marketBalance(market, account0), {
          timeout: 120_000,
        })
        .toBe(0n)
      // Optional dismiss: the button can be visible yet pointer-blocked by a closing overlay —
