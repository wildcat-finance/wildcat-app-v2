...
test.describe.serial("lender flows: ToU / privacy / MLA agreements", () => {
  ...
  test("LEN-03: ToU state across lender/borrower sides is capacity-scoped and never traps", async ({
    ...
    await step(
      page,
      "borrower side: no first-acceptance prompt; the page is reachable (not a trap)",
      async () => {
        // Actual behavior: no redirect for a non-borrower wallet; the borrower home renders.
        // `ToUReacceptanceModal` auto-opens for staleExpired / staleWithinGrace / declined ONLY
        // (`autoOpen` in that component) and has no "Terms of Use Signature Required" title at
        // all, and there is no borrower-invitation lookup, so the wallet simply lands on the
        // borrower home with no prompt — which satisfies the point of the case (never trapped).
        //
        // No auto-prompt, and nothing blocking: the page is interactive and the header still
        // offers the way back. Give the modal the same window it would have had to appear in.
        await page.waitForTimeout(5_000)
        await expect(dialog).toHaveCount(0)
        await expect(
          page.getByRole("link", { name: "Lender" }).first(),
        ).toBeVisible({ timeout: 30_000 })
      },
    )
  ...
  test("LEN-13: MLA market — deposit gated until the MLA is countersigned; signed MLA downloadable", async ({
    ...
    await step(
      ...
      async () => {
        ...
        // Copy drift, same sentence: v2.5 "Sign the MLA before depositing into this market.";
        // main "You need to sign the MLA before you can deposit into this market."
        await expect(
          page.getByText(
            /sign the MLA before (depositing into|you can deposit into) this market/i,
          ),
        ).toBeVisible()
