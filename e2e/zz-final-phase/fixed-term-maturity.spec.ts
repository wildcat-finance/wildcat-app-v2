 ...
 * (e2e/CONVENTIONS.md "Structure"), so "LEN-35b: after maturity, …" is the `b` half of the LEN-35
...
test.describe.serial("final phase: fixed-term maturity transition (LEN-35)", () => {
  ...
  test("LEN-35b: after maturity, withdrawal requests work", async ({
    ...
    // at the amount needed. The pinned fallback fixture has no minimum, which is why a flat
    // one-unit gap survived here for as long as this test ran only against it; every market the
    // creation wizard deploys sets one (100 units), so the constant had to go.
    {
      const gap =
