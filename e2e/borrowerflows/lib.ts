...
import { opsBorrower } from "./helpers"
import {
...
/**
 * The borrower the ops suite acts as: a real, pre-fork Sepolia borrower driven through anvil
 * impersonation — the borrower market page cannot render a market the fork itself created
 * (see `helpers.MAIN_OPS_BORROWER`).
 */
export const BORROWER = opsBorrower
 ...
 * landed" from "the indexer is behind". Works on both generations (identical struct).
 */
...
const MARKET_FIELDS = `
  id name symbol isClosed isDelinquent isIncurringPenalties
 ...
 *
 * The v2.1.8 subgraph has no `providerInstance` edge, so the two facts are resolved on-chain:
 * only an access-list provider implements `administrator()` — every other role provider reverts —
 * and its return value IS the administrator.
 */
export const accessListProviders = async (hooks: string) => {
  const { roleProviders } = await gql<{
    roleProviders: {
    ...
    `{ roleProviders(where: { hooks: "${hooks.toLowerCase()}", isApproved: true }) {
        providerAddress isApproved } }`,
  )

  const owned: typeof roleProviders = []
  for (const p of roleProviders) {
    const administrator = await chain.publicClient
      .readContract({
        address: p.providerAddress as Address,
        abi: accessListProviderAbi,
        functionName: "administrator",
      })
      .catch(() => undefined)
    if (administrator?.toLowerCase() === BORROWER.toLowerCase()) owned.push(p)
  }
  return owned
}
...
export const feesCollectedRecords = async (market: string) =>
  (
    await gql<{
      feesCollecteds: { feesCollected: string }[]
    }>(
      `{ feesCollecteds(where: { market: "${market.toLowerCase()}" }, orderBy: blockTimestamp, orderDirection: desc, first: 5) { feesCollected } }`,
    )
  ).feesCollecteds
...
/**
 * Is the account a member of the role provider (ACCESS_LIST edits)?
 *
 * v2.1.8 has no `RoleProviderMember` entity, so this reads the provider contract — the same fact,
 * read from the source the subgraph would have indexed.
 */
