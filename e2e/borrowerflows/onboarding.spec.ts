...
// The ArchController's owner is resolved FROM THE CHAIN (helpers.archControllerOwner), not from
// the SDK deployment manifest: SDK 3.1.17 still names a superseded MockArchControllerOwner, so on
// main every registerBorrower/removeBorrower routed through the manifest address reverts.
...
const deregisterBorrowerOnChain = async (account: Address) => {
  ...
  await waitTx(
    await chain.walletFor(owner).writeContract({
      address: ARCH_CONTROLLER,
      abi: archAbi,
 ...
 *  and dismiss it deterministically (see assertNoTouPromptAndClear) before role-based asserts. */
const dismissTouPromptIfOpen = async (page: Page) => {
  ...
  // stamped and the prompt can no longer auto-open.
  const dialog = page.getByRole("dialog")
  if (!(await dialog.isVisible().catch(() => false))) return
  // The FIRST dialog on a public borrower profile is not the ToU prompt but the KYB explainer
  // ("How Wildcat checks this profile", VerificationDisclosure). That Dialog is rendered with no
  // onClose, so Escape does nothing and the aria-hidden app root never comes back — it has to be
  // acknowledged through its own "I understand" button.
  const acknowledge = page.getByRole("button", { name: "I understand" })
  if (await acknowledge.isVisible().catch(() => false)) {
    await acknowledge.click()
    await expect(acknowledge).toBeHidden({ timeout: 15_000 })
    if (!(await dialog.isVisible().catch(() => false))) return
  }
...
/**
 * Assert that NO first-acceptance ToU prompt auto-opens, and clear any dialog that is open.
 *
 * `ToUReacceptanceModal` auto-opens for staleExpired / staleWithinGrace / declined ONLY (its
 * `autoOpen`), and its title is one of "Updated Terms of Use" / "Terms of Use" / "Terms of Use
 * Declined". The first-acceptance titles this suite names ("Terms of Use Signature Required",
 * "Borrower Invitation Required") and the invitation lookup that would choose between them exist
 * nowhere in the tree, so a never-signed borrower simply gets the page with no prompt.
 *
 * The call sites need the page free of an open MUI dialog (an open dialog aria-hides everything
 * behind it, so role queries find nothing), so this also clears whatever is on screen.
 */
const assertNoTouPromptAndClear = async (
  page: Page,
  title: RegExp | string,
) => {
  const dialog = page.getByRole("dialog")
  // Give the prompt the same window it would have had, then assert it never came.
...
test.describe.serial("borrower onboarding (BON-01…10)", () => {
  ...
  test("BON-01: never-invited wallet sees the get-in-touch onboarding screen", async ({
    ...
    await step(
      ...
      async () => {
        // A never-signed Borrower-party wallet gets NO first-signature prompt (see
        // assertNoTouPromptAndClear); BON-01 must not sign anything for #5 — assert that and
        // clear any dialog. (Role queries below need the page free of one anyway.)
        await assertNoTouPromptAndClear(page, "Terms of Use Signature Required")
      },
    )
  ...
  test("BON-02: invited wallet sees the pending invitation with an explicit Accept", async ({
    ...
    await step(
      page,
      "no ToU prompt auto-opens; the dashboard banner is the route into the invitation",
      async () => {
        // See assertNoTouPromptAndClear: the ToU modal has no first-acceptance state and no
        // borrower-invitation lookup, so nothing auto-opens for an invited-but-unsigned
        // borrower and the dashboard banner below is the only route into the ceremony.
        await assertNoTouPromptAndClear(page, "Borrower Invitation Required")
      },
    )
  ...
  test("BON-03: accepting the invitation records the borrower's ToU acceptance", async ({
    ...
    await step(page, "sign in (wallet login signature)", async () => {
      // Fresh context: no invitation-required ToU prompt auto-opens here either — assert that
      // and clear any dialog (an open one aria-hides the page beneath from role queries).
      await assertNoTouPromptAndClear(page, "Borrower Invitation Required")
  ...
  test("BON-06: ToU document downloads from the signing screen and the profile", async ({
    ...
    // The certificate route always writes the terms file and carries no availability flag. The
    // substantive check is that the file is in the ZIP and hashes to the accepted version.
  ...
  test("BON-08: borrower-side profile edit renders on the public profile", async ({
    ...
    await step(page, "edit description and website", async () => {
      ...
      // fetch-triggered reset would overwrite the input.
      await expect(descriptionField).toHaveValue(INVITE_DESCRIPTION, {
        timeout: 60_000,
      })
  ...
  test("BON-09: ToU version bump — re-sign prompt states and blocked actions", async ({
    ...
    try {
      ...
      await step(page, "state 3: create-market is blocked", async () => {
        // NOT gotoGatedBorrowerPath here: state 2 leaves a NON-dismissible modal up, and MUI
        // aria-hides the app root beneath it, so that helper's `ensureConnected` can never see
        // the header's connect/address button and waits out the whole test timeout. The wallet is
        // already connected in this context (connectAs seeded the wagmi cookie above) and the
        // assertions below are text queries, which are not filtered by the accessibility tree.
      ...
      await step(page, "re-sign from the blocking prompt", async () => {
        ...
        // Copy drift: v2.5 "…as {party} on this network - this capacity is up to date.",
        // main "You have accepted the current Wildcat Terms of Use - you are up to date."
