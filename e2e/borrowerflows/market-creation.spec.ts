...
test.describe.serial("borrower flows: market creation (MKT-01…24)", () => {
  ...
  test("MKT-03: every open-term parameter settable; review units and formatting; on-chain agreement", async () => {
    ...
    // Review formatting: thousands separators; durations are humanized on head (a5620443
    // "unify duration units on confirmation screen" + 3d68d06f "stop rounding review
    // durations"): "1.00 hours" -> "1 hour".
    expect(reviewD1["Maximum Borrowing Capacity"]).toContain("1,000,000")
    expect(reviewD1["Maximum Borrowing Capacity"]).toContain(asset.symbol)
    expect(reviewD1["Grace Period Duration"]).toBe("1 hour")
    expect(reviewD1["Withdrawal Cycle Duration"]).toBe("1 hour")
  ...
  test("MKT-05: revolving market — commitment fee + utilisation APR; 0% reserve ratio", async ({
    page,
  }) => {
    await connectAs(page, 3)
    await gotoCreateMarket(page)
  ...
  test("MKT-06: periodic-term market — schedule fields + unit toggle", async ({
    page,
  }) => {
    await connectAs(page, 3)
    await gotoCreateMarket(page)
  ...
  test("MKT-07: revolving + periodic combined type deploys with both parameter sets", async ({
    page,
  }) => {
    await connectAs(page, 3)
    await gotoCreateMarket(page)
  ...
  test("MKT-09: periodic timing violations rejected at entry; Tab/Enter cannot bypass", async ({
    page,
  }) => {
    await connectAs(page, 3)
    await gotoCreateMarket(page)
  ...
  test("MKT-21: new markets appear on the borrower overview; indexing lag recorded", async ({
    ...
    await expect
      .poll(
        ...
        { timeout: 120_000, message: "newest market visible on the overview" },
      )
      .toBe(true)
    await expect(page.getByText(d1.marketName).first()).toBeVisible({
      timeout: 60_000,
  ...
  test("MKT-23: market without wrapper offers post-hoc deployment on both sides", async ({
    page,
  }) => {
